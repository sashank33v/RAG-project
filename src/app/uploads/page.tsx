...
export default function UploadsPage() {
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      setUploadedFiles((prev) => [{ name: res.data.filename, status: "Ready" }, ...prev]);
      setFile(null);
        ...
        <section className="flex-1">
          ...
          <div className="px-6 py-8 lg:px-8">
            <div className="mx-auto max-w-[1400px]">
              <div className="mb-8">
                <p className="text-[11px] uppercase tracking-[0.28em] text-zinc-500">Uploads</p>
                <h1 className="mt-2 text-3xl font-semibold tracking-tight">Knowledge Upload Center</h1>
              </div>

              <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
                <section className="rounded-[28px] border border-white/10 bg-[#111318] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
                  <p className="text-[11px] uppercase tracking-[0.22em] text-zinc-500">Upload</p>
                  <h2 className="mt-2 text-2xl font-semibold text-white">Add your files</h2>
                ...
                <section className="rounded-[28px] border border-white/10 bg-[#111318] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
                  <div className="mb-6 flex items-center justify-between">
                    <div>
                      <p className="text-[11px] uppercase tracking-[0.22em] text-zinc-500">Library</p>
                      <h2 className="mt-2 text-2xl font-semibold text-white">Uploaded files</h2>
                    </div>
                  ...
                  {uploadedFiles.length === 0 ? (
                    <div className="flex min-h-[320px] items-center justify-center rounded-[24px] border border-dashed border-white/10 bg-[#0d1016]">
                      <div className="text-center">
                        <p className="text-lg font-medium text-zinc-300">No files uploaded yet</p>
                        <p className="mt-2 text-sm text-zinc-500">Upload your first file from the panel on the left.</p>
                      </div>
                    </div>
                  ) : (
                        ...
                        <div key={index} className="rounded-[22px] border border-white/10 bg-[#0d1016] p-5">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="mt-2 text-xs text-zinc-500">Ready for processing</p>
                            </div>
